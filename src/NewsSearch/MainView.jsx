...
function MainView({searchData}) {
  ...
  let prefix = '';
  let postfix = searchData.slice(0,searchData.length - 1);



  if(searchData.slice(searchData.length-1,searchData.length) === '1') {
    prefix = 'posts/news';

  } else if(searchData.slice(searchData.length-1,searchData.length) === '2') {
    prefix = 'posts/calender';
  }

    const url = `${prefix}?search=${postfix}`;
    // 
  ...
  useEffect(() => {
    ...
    return (
        ...
        <div className=" mt-10 lg:mt-14 w-full grid grid-cols-1 lg:grid-cols-[0.5fr_1fr] lg:gap-x-16 gap-10 lg:py-[72px] py-14 border-t-2 border-[#F7F7F7]">
          {(data.length > 0) && <h5 className="font-normal leading-7 text-xl text-[#1C1D2080]">Search results {currentPage  + 1} - {currentPage + data.length} of total {data.length} for <span className="text-[#1c1d20] ">{postfix}</span></h5>}
        ...
        <div className="flex flex-col gap-6">
            {
                currentItems.map((item, index) => 
                    <Link to= {`/about/news/${item.id}`} >
                        ...
                        <h5 className="font-normal lg:leading-7 text-base leading-[22px] lg:text-xl me-5 hover:hoverMode">{item.title}</h5>
